...
public class ReportServiceImplementation implements ReportService {
    ...
    @Autowired
    public ReportServiceImplementation(TransactionService transactionService, FxRatesService fxRatesService) {
    ...
    public ReportDTO generateReport(String interval, String currency) {
        ...
        transactions.forEach(t -> {
            double convertedAmount = getConvertedAmount(t, exchangeRates, requiredCurrencyName.name());
    ...
    private double getConvertedAmount(TransactionDTO transaction, Map<String, Double> exchangeRates, String targetCurrency) {
        String transactionCurrency = transaction.getCurrencyName().toString();

        if (!exchangeRates.containsKey(transactionCurrency)) {
            throw new IllegalArgumentException("Exchange rate not found for: " + transactionCurrency);
        }
